use rkvs::{Result, StorageManager, NamespaceConfig, StorageConfig, ManagerAutosaveConfig, NamespaceAutosaveConfig};
...
async fn test_save_and_load_manager_snapshot() -> Result<()> {
    ...
    // --- Phase 1: Create data and save it ---
    {
        ...
        storage1.create_namespace(ns1_name, Some(ns1_config)).await?;

        let ns2_config = NamespaceConfig::default();
        ns2_config.set_shard_count(4);
        storage1.create_namespace(ns2_name, Some(ns2_config)).await?;
        ...
        assert_eq!(namespace1.get(ns1_key).await.map(|v| (*v).clone()), Some(ns1_value.to_vec()));
        assert_eq!(namespace2.get(ns2_key).await.map(|v| (*v).clone()), Some(ns2_value.to_vec()));
        ...
        assert_eq!(loaded_ns1.get(ns1_key).await.map(|v| (*v).clone()), Some(ns1_value.to_vec()));

        let loaded_ns2 = storage2.namespace(ns2_name).await?;
        assert_eq!(loaded_ns2.get(ns2_key).await.map(|v| (*v).clone()), Some(ns2_value.to_vec()));
...
async fn test_save_and_load_single_namespace_snapshot() -> Result<()> {
    ...
    // --- Phase 1: Create data and save a single namespace ---
    {
        ...
        storage1.save_namespace(ns1_name, ns_snapshot_filename).await?;
    }
...
async fn test_load_namespace_with_new_name() -> Result<()> {
    ...
    // --- Phase 1: Create and save a single namespace snapshot ---
    {
        ...
        storage1.create_namespace(original_ns_name, Some(ns_config)).await?;
        let namespace = storage1.namespace(original_ns_name).await?;
        namespace.set(key, value.to_vec()).await?;

        // Save the snapshot
        storage1.save_namespace(original_ns_name, snapshot_filename).await?;
    }
    ...
    // --- Phase 2: Load the snapshot multiple times with new names ---
    {
        ...
        let loaded_name_1 = storage2.load_namespace_with_name(snapshot_filename, new_name_1).await?;
        assert_eq!(loaded_name_1, new_name_1);

        // Load the second clone
        let loaded_name_2 = storage2.load_namespace_with_name(snapshot_filename, new_name_2).await?;
        ...
        assert_eq!(ns1.get(key).await.map(|v| (*v).clone()), Some(value.to_vec()));
        assert_eq!(ns1.get_config().await.shard_count(), 2);

        // Verify second clone
        let ns2 = storage2.namespace(new_name_2).await?;
        assert_eq!(ns2.get(key).await.map(|v| (*v).clone()), Some(value.to_vec()));
